...
struct VM {
    ...
    int relbase;
    int ip;
    int status; // 0: reset, 1: running, 2:input, 3:stopped, 4:output
...
int main(int argc, char **argv) {
    ...
    while (vm->status != 3) {
        switch (vm->status) {
            default: quit("bad status"); break;
            case 0: quit("0: bad status"); break;
            case 1: quit("1: bad status"); break;
            case 2: quit("2: bad status, no input for day 13, part 1");
                    //vm->smallsignal = 2;
                    //vm->smallsignal = 0; mpz_set_ui(vm->bigsignal, 1);
                    break;
            case 3: break;
            case 4: gmp_printf("program output: %d\n", vm->smallsignal);
                    break;
        }
        execute(vm);
    }
